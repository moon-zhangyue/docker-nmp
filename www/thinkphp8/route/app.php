...
// Route::get('goods/autocomplete', 'goods/autocomplete'); // 自动补全接口

Route::group('goods', function () {
    Route::get('sync', 'goods/sync'); // 批量同步（用于初始化或数据修复）
    Route::post('search', 'goods/search'); // 全文商品搜索接口
    Route::post('filter', 'goods/filter'); // 按价格范围和分类过滤
    Route::post('aggregations', 'goods/aggregations'); // 商品聚合接口
    Route::post('import-to-es', 'goods/importGoodsToEs'); // 导入数据库商品数据到Elasticsearch
    Route::get('autocomplete', 'goods/autocomplete'); // ES商品名称自动补全接口
    Route::get('updateMapping', 'goods/updateMapping'); // 更新索引映射，确保name_suggest字段被正确定义为completion类型
});
