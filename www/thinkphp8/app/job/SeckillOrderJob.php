    ...
    public function fire(Job $job, array $data): void
    {
        try {
            ...
            try {
                ...
                $seckillOrder->order_sn     = $orderSn;
                $seckillOrder->user_id      = $userId;
                $seckillOrder->activity_id  = $activityId;
                $seckillOrder->goods_id     = $goodsId;
                $seckillOrder->sku_id       = $skuId;
                $seckillOrder->quantity     = $quantity;
                $seckillOrder->price        = $price;
                $seckillOrder->total_amount = $price * $quantity;
                $seckillOrder->status       = SeckillOrder::STATUS_PENDING;
                $seckillOrder->save();

                // 兼容旧版本 - 创建普通订单
                if (Db::name('order')->getTable()) {
                    // 创建订单
                ...
                if ($goodsId > 0) {
                    ...
                    if ($seckillGoods) {
                        ...
                        if ($remainStock !== false && is_numeric($remainStock)) {
                            $seckillGoods->remain_stock = max(0, (int) $remainStock);
                            $seckillGoods->save();
                        }
                    }
                }
