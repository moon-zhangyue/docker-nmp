...
class IoTDataShardedService
{
    ...
    public function getDeviceAnalysis(array $deviceIds, array $timeRange = []): array
    {
        try {
            ...
            Log::info('设备分析完成', ['device_count' => count($deviceIds)]);
            return $result;
    ...
    public function performDataArchive(array $archiveOptions = []): array
    {
        try {
            ...
            if ($success) {
                ...
                Log::info('数据归档完成', ['archive_days' => $archiveDays]);
                return [
                    'success' => true,
                    'data' => [
                        'archive_days' => $archiveDays,
    ...
    public function getShardHealthStatus(): array
    {
        try {
            ...
            Log::error('获取分片健康状态失败', ['error' => $e->getMessage()]);
    ...
    private function updateStatisticsCache(array $processedData): void
    {
        try {
            ...
            Log::info('统计缓存更新完成', ['device_count' => count($deviceCounts)]);
        } catch (\Exception $e) {
            Log::warning('更新统计缓存失败', ['error' => $e->getMessage()]);
        }
    }
    ...
    private function triggerAnalysisTasks(array $processedData): void
    {
        try {
            ...
            if (count($processedData) > 1000) {
                // 这里应该加入队列任务
                Log::info('触发异步分析任务', ['data_count' => count($processedData)]);
            }
        } catch (\Exception $e) {
            Log::warning('触发分析任务失败', ['error' => $e->getMessage()]);
        }
    }
    ...
    private function clearStatisticsCache(): void
    {
        try {
            ...
            Log::warning('清除统计缓存失败', ['error' => $e->getMessage()]);
        }
    }
}