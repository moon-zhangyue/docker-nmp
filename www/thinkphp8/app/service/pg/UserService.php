...
class UserService
{
    ...
    public function register(array $data)
    {
        ...
        if (User::where('username', $data['username'])->find()) {
            Log::error('用户注册失败：用户名已存在', ['username' => $data['username']]);
            throw new BusinessException('用户名已存在');
        }
        
        // 验证邮箱是否已存在
        if (User::where('email', $data['email'])->find()) {
            Log::error('用户注册失败：邮箱已存在', ['email' => $data['email']]);
            throw new BusinessException('邮箱已存在');
        }
        
        // 验证手机号是否已存在
        if (isset($data['mobile']) && $data['mobile'] && User::where('mobile', $data['mobile'])->find()) {
            Log::error('用户注册失败：手机号已存在', ['mobile' => $data['mobile']]);
        ...
        try {
            ...
            Log::info('用户注册成功', ['user_id' => $user->id, 'username' => $user->username]);
            
            return $user;
        } catch (\Exception $e) {
            Log::error('用户注册异常', ['error' => $e->getMessage(), 'data' => $data]);
    ...
    public function login(string $username, string $password)
    {
        ...
        // 用户不存在
        if (!$user) {
            Log::warning('用户登录失败：用户不存在', ['username' => $username]);
            throw new BusinessException('用户名或密码错误');
        }
        
        // 用户被禁用
        if ($user->status != 1) {
            Log::warning('用户登录失败：用户已被禁用', ['user_id' => $user->id, 'username' => $user->username]);
            throw new BusinessException('账号已被禁用，请联系管理员');
        }
        
        // 验证密码
        if (!$user->validatePassword($password)) {
            Log::warning('用户登录失败：密码错误', ['user_id' => $user->id, 'username' => $user->username]);
            throw new BusinessException('用户名或密码错误');
        }
    ...
    public function getUserInfo(int $userId)
    {
        ...
        if (!$user) {
            Log::warning('获取用户信息失败：用户不存在', ['user_id' => $userId]);
    ...
    public function updateUserInfo(int $userId, array $data)
    {
        ...
        if (!$user) {
            Log::warning('更新用户信息失败：用户不存在', ['user_id' => $userId]);
            throw new BusinessException('用户不存在');
        }
        ...
        if (isset($data['mobile']) && $data['mobile'] && $data['mobile'] != $user->mobile) {
            if (User::where('mobile', $data['mobile'])->where('id', '<>', $userId)->find()) {
                Log::warning('更新用户信息失败：手机号已存在', ['user_id' => $userId, 'mobile' => $data['mobile']]);
        ...
        try {
            ...
            Log::info('更新用户信息成功', ['user_id' => $userId]);
            
            return $user;
        } catch (\Exception $e) {
            Log::error('更新用户信息异常', ['error' => $e->getMessage(), 'user_id' => $userId, 'data' => $data]);
    ...
    public function changePassword(int $userId, string $oldPassword, string $newPassword)
    {
        ...
        if (!$user) {
            Log::warning('修改密码失败：用户不存在', ['user_id' => $userId]);
            throw new BusinessException('用户不存在');
        }
        
        // 验证旧密码
        if (!$user->validatePassword($oldPassword)) {
            Log::warning('修改密码失败：旧密码错误', ['user_id' => $userId]);
        ...
        try {
            ...
            Log::info('修改密码成功', ['user_id' => $userId]);
            
            return $result;
        } catch (\Exception $e) {
            Log::error('修改密码异常', ['error' => $e->getMessage(), 'user_id' => $userId]);
    ...
    public function addAddress(int $userId, array $data)
    {
        // 检查用户是否存在
        if (!User::find($userId)) {
            Log::warning('添加用户地址失败：用户不存在', ['user_id' => $userId]);
            throw new BusinessException('用户不存在');
        }
        ...
        try {
            ...
            Log::info('添加用户地址成功', ['user_id' => $userId, 'address_id' => $address->id]);
            
            return $address;
        } catch (\Exception $e) {
            Db::rollback();
            Log::error('添加用户地址异常', ['error' => $e->getMessage(), 'user_id' => $userId, 'data' => $data]);
    ...
    public function updateAddress(int $userId, int $addressId, array $data)
    {
        ...
        if (!$address) {
            Log::warning('更新用户地址失败：地址不存在或不属于该用户', ['user_id' => $userId, 'address_id' => $addressId]);
        ...
        try {
            ...
            Log::info('更新用户地址成功', ['user_id' => $userId, 'address_id' => $addressId]);
            
            return $address;
        } catch (\Exception $e) {
            Db::rollback();
            Log::error('更新用户地址异常', ['error' => $e->getMessage(), 'user_id' => $userId, 'address_id' => $addressId, 'data' => $data]);
    ...
    public function deleteAddress(int $userId, int $addressId)
    {
        ...
        if (!$address) {
            Log::warning('删除用户地址失败：地址不存在或不属于该用户', ['user_id' => $userId, 'address_id' => $addressId]);
            throw new BusinessException('地址不存在或不属于您');
        }
        ...
        try {
            ...
            Log::info('删除用户地址成功', ['user_id' => $userId, 'address_id' => $addressId]);
            
            return $result;
        } catch (\Exception $e) {
            Db::rollback();
            Log::error('删除用户地址异常', ['error' => $e->getMessage(), 'user_id' => $userId, 'address_id' => $addressId]);
    ...
    public function setDefaultAddress(int $userId, int $addressId)
    {
        ...
        if (!$address) {
            Log::warning('设置默认地址失败：地址不存在或不属于该用户', ['user_id' => $userId, 'address_id' => $addressId]);
            throw new BusinessException('地址不存在或不属于您');
        }
        ...
        try {
            ...
            Log::info('设置默认地址成功', ['user_id' => $userId, 'address_id' => $addressId]);
            
            return $result;
        } catch (\Exception $e) {
            Log::error('设置默认地址异常', ['error' => $e->getMessage(), 'user_id' => $userId, 'address_id' => $addressId]);
